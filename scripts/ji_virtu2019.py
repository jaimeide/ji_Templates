...
def p5_helper_print_binStats(dfx,label):
    try:
        ...
        print('(*) Warning! Could not print the Bin stats maybe of insufficient number of bins. Check...')
...
def p5_wrapper_generate_plots_April3_notTraded(df2plot,set2run,alllabels,quantiles = [0.2, 0.4, 0.6, 0.8, 0.9, 0.95], sizeVar = 'adjSizeShare',sizeLabel = 'adjSizeShare', condVarX = 'pred30min', condLabelX = 'predVolaSur30min', savepath = './', allsavelabel=None):

    ## 2) Get masks
    allTradeLabels = []
    allmasks = []
    allTradeLabels.append('notTraded')
    allmasks.append(df2plot.FillNbrForBin == 0)  # Will include everything
    ...
    for rangeHLG, gLabel, savelabel in zip(set2run, alllabels, allsavelabel):
        ...
        # IMPORTANT: SaveLabels should have the same order as allTradeLabels!
        allSaveLabels = []
        allSaveLabels.append('NotTradedBins_%s' % savelabel)
    ...
    for yy in allyy:
        for qq in allqq:
            if nn.nrows is not None:
                ...
                print(f'- Loading clustersABD_ALL_{iso}_{yy}{qq}.dat.gz...')
                #dfx = pd.read_csv(Path.joinpath(dpath, f'clustersABD_ALL_{iso}_{yy}{qq}.dat.gz'), compression='gzip',
                #                  sep=',', usecols=colsToReadx, na_filter=False)
                dfx = pd.read_csv(os.path.join(dpath, f'clustersABD_ALL_{iso}_{yy}{qq}.dat.gz'), compression='gzip',
                                                    sep=',', usecols=colsToReadx, na_filter=False)
